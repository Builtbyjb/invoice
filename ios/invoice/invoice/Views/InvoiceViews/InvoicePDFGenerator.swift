...
struct InvoicePDFGenerator {
    ...
    static func generatePDF(for invoice: Invoice) -> Data? {
        ...
        let renderer = UIGraphicsPDFRenderer(
            bounds: CGRect(x: 0, y: 0, width: a4Width, height: a4Height),
            format: format
        )
    ...
    private static func drawHeader(context: UIGraphicsPDFRendererContext, invoice: Invoice, startY: CGFloat) -> CGFloat
    {
        ...
        businessName.draw(
            at: CGPoint(x: margin + logoSize + 12, y: startY + 4),
            withAttributes: [
                .font: businessFont,
    ...
    private static func drawMetaInfo(context: UIGraphicsPDFRendererContext, invoice: Invoice, startY: CGFloat)
        -> CGFloat
    {
        ...
        for (index, item) in metaItems.enumerated() {
            ...
            item.1.draw(
                at: CGPoint(x: x, y: startY + titleSize.height + 2),
                withAttributes: [
                    .font: valueFont,
                    .foregroundColor: UIColor.label,
                ]
    ...
    private static func drawClientInfo(context: UIGraphicsPDFRendererContext, invoice: Invoice, startY: CGFloat)
        -> CGFloat
    {
        let title = "Billed To"
        let titleFont = UIFont.systemFont(ofSize: 10, weight: .semibold)
    ...
    private static func drawLineItems(context: UIGraphicsPDFRendererContext, invoice: Invoice, startY: CGFloat)
        -> CGFloat
    {
        let tableX = margin
        ...
        for (index, header) in headers.enumerated() {
            header.draw(
                at: CGPoint(x: colX + 6, y: startY + (headerHeight - headerFont.lineHeight) / 2),
                withAttributes: [
                    .font: headerFont,
                    .foregroundColor: UIColor.secondaryLabel,
                ]
        ...
        for item in invoice.lineItems {
            ...
            (currencyFormatter.string(from: NSNumber(value: item.price)) ?? "\(item.price)").draw(
                in: priceRect,
                withAttributes: numberAttributes
            )

            let totalRect = CGRect(
                x: tableX + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3],
                y: textY,
                width: colWidths[4] - 6,
                height: rowHeight
            )
            (currencyFormatter.string(from: NSNumber(value: item.total)) ?? "\(item.total)").draw(
                in: totalRect,
                withAttributes: numberAttributes
            )
    ...
    private static func drawSummary(context: UIGraphicsPDFRendererContext, invoice: Invoice, startY: CGFloat) -> CGFloat
    {
        ...
        for (label, value, valueFont) in summaryItems {
            ...
            valueString.draw(
                at: CGPoint(x: valueX + 80 - valueSize.width, y: cursorY),
                withAttributes: [
                    .font: valueFont,
                    .foregroundColor: UIColor.label,
                ]
    ...
    private static func drawSignature(context: UIGraphicsPDFRendererContext, invoice: Invoice, startY: CGFloat)
        -> CGFloat
    {
        let title = "Signature"
        let titleFont = UIFont.systemFont(ofSize: 10, weight: .semibold)
